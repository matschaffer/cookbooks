...
if node[:ganglia][:unicast]
  unicast_targets = all_providers_for_service("gmond-#{node[:ganglia][:cluster_name]}")
else
  unicast_targets = []
end
provide_service("gmond-#{node[:ganglia][:cluster_name]}")

template "/etc/ganglia/gmond.conf" do
  source "gmond.conf.erb"
  variables :unicast_targets => unicast_targets
